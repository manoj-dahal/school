(function () {
  ...
  sInput?.addEventListener("input", () => {
    ...
    sTimer = setTimeout(async () => {
      try {
        ...
        const bits = [];
        (data.pages || []).forEach(p => bits.push(`<a href="${p.path}"><div><div class="k">Page</div><strong>${p.title}</strong><div class="small">${p.hint}</div></div></a>`));
        (data.news || []).forEach(n => bits.push(`<a href="/news/${n.slug}"><img src="${n.cover}" alt=""><div><div class="k">News</div><strong>${n.title}</strong><div class="small">${n.date_bs}</div></div></a>`));
        (data.staff || []).forEach(s => bits.push(`<a href="/family"><img src="${s.image || ""}" alt=""><div><div class="k">Staff</div><strong>${s.name}</strong><div class="small">${s.role}${s.subject ? " · " + s.subject : ""}</div></div></a>`));
  ...
  if (faqRoot && !faqRoot.children.length) {
    fetch("/api/faq").then(r => r.json()).then(items => {
  ...
  /* Gallery: keep baked-in photos; API is optional enhancement */
  function escAttr(s) {
  ...
  if (galRoot) {
    ...
    (async () => {
      const tries = ["/api/gallery", BASE + "assets/data/gallery.json", "/assets/data/gallery.json"];
  ...
  if (PAGE === "home") {
    fetch("/api/news").then(r => r.json()).then(items => {
      ...
      const links = items.slice(0, 6).map(n =>
        `<a class="ticker-item" href="/news/${n.slug}/index.html">${n.title}<span> — ${n.date_bs}</span></a><span class="ticker-sep" aria-hidden="true">·</span>`
      ).join("");
  ...
  if (ideaBoard) {
    fetch("/api/suggestions/public").then(r => r.json()).then(items => {
